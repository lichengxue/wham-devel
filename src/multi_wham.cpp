...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_INTEGER(apply_mu_trend); // trend on mean mu
  DATA_SCALAR(trend_mu_rate); // rate of trend on mean mu
  ...
  DATA_INTEGER(move_dyn); // movement dynamics (0 = natal homing, 1 = meta-population)
  ...
  if(n_regions>1){//only do this mess if the number of regions is greater than 1.
    ...
    SIMULATE if(do_simulate_mu_re){
      mu_re = simulate_mu_re(mu_repars, mu_re, mu_model, can_move, years_use, apply_re_trend, trend_re_rate);
      REPORT(mu_re);
    }
